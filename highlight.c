...
static void pos_adjust( rpl_highlight_env_t* henv, ssize_t* ppos, ssize_t* plen ) {
  ssize_t pos = *ppos;
  ssize_t len = *plen;
...
static void highlight_attr(rpl_highlight_env_t* henv, ssize_t pos, ssize_t count, attr_t attr ) {
  if (henv==NULL) return;
...
rpl_public void rpl_highlight(rpl_highlight_env_t* henv, long pos, long count, const char* style ) {
  if (henv == NULL || style==NULL || style[0]==0 || pos < 0) return;  
  highlight_attr(henv,pos,count,bbcode_style( henv->bbcode, style ));
}

rpl_public void rpl_highlight_formatted(rpl_highlight_env_t* henv, const char* s, const char* fmt) {
