...
rpl_private ssize_t  history_count(const history_t* h) {
  return h->count;
}
...
rpl_private bool history_update( history_t* h, const char* entry ) {
...
rpl_private const char* history_get_with_prefix(const history_t* h, ssize_t from, const char* prefix) {
  const char* p = NULL;
  ssize_t i;
...
rpl_private bool history_search( const history_t* h, ssize_t from /*including*/, const char* search, bool backward, ssize_t* hidx, ssize_t* hpos ) {
...
rpl_private void history_load_from(history_t* h, const char* fname, long max_entries ) {
  ...
  if (max_entries < 0 || max_entries > RPL_MAX_HISTORY) max_entries = RPL_MAX_HISTORY;
...
static bool rpl_isxdigit( int c ) {
