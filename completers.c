...
typedef struct word_closure_s {
  ...
  void*                 prev_env;
  rpl_completion_fun_t*  prev_complete;
} word_closure_t;


// word completion callback
static bool token_add_completion_ex(rpl_env_t* env, void* closure, const char* replacement, const char* display, const char* help, long delete_before, long delete_after) {
...
rpl_public void rpl_complete_word(rpl_completion_env_t* cenv, const char* prefix, rpl_completer_fun_t* fun,
                                    rpl_is_char_class_fun_t* is_word_char) 
{
  if (is_word_char == NULL) is_word_char = &rpl_char_is_nonseparator;
...
// word completion callback
static bool qword_add_completion_ex(rpl_env_t* env, void* closure, const char* replacement, const char* display, const char* help, 
...
rpl_public void rpl_complete_qword( rpl_completion_env_t* cenv, const char* prefix, rpl_completer_fun_t* fun, rpl_is_char_class_fun_t* is_word_char ) {
  rpl_complete_qword_ex( cenv, prefix, fun, is_word_char, '\\', NULL);
}


rpl_public void rpl_complete_qword_ex( rpl_completion_env_t* cenv, const char* prefix, rpl_completer_fun_t* fun, 
  ...
  if (quote == 0) {
    ...
    while (wpos < wlen) {
      ...
      {
        rpl_memmove(word + wpos, word + wpos + 1, wlen - wpos /* including 0 */);
      }
      wpos += ofs;
    }
  }
...
rpl_private char rpl_dirsep(void) {
  return '\\';
}
#else
...
static bool ends_with_n(const char* name, ssize_t name_len, const char* ending, ssize_t len) {
  ...
  for (ssize_t i = 1; i <= len; i++) {
    ...
    #ifdef _WIN32
    if (rpl_tolower(c1) != rpl_tolower(c2)) return false;
    #else
...
static bool filename_complete_indir( rpl_completion_env_t* cenv, stringbuf_t* dir, 
    ...
    do {
          ...
          rpl_starts_with(name, base_prefix))
      {
        ...
        { // check directory and potentially add a dirsep to the dir_prefix
          ...
          sbuf_append_char(dir,rpl_dirsep());
        ...
        if (isdir || match_extension(name, extensions)) {
          ...
          cont = rpl_add_completion_ex(cenv, sbuf_string(dir_prefix), sbuf_string(display), NULL);
        }
...
static void filename_completer( rpl_completion_env_t* cenv, const char* prefix ) {
...
rpl_public void rpl_complete_filename( rpl_completion_env_t* cenv, const char* prefix, char dir_sep, const char* roots, const char* extensions ) {
  if (roots == NULL) roots = ".";
