...
static bool
token_add_completion_ex(rpl_env_t * env, void *closure, const char *replacement,
                        ...
                        long delete_before, long delete_after)
{
	...
	return (*wenv->prev_complete) (env, wenv->prev_env, replacement, display,
	                               help,
...
rpl_public void
rpl_complete_word(rpl_completion_env_t * cenv, const char *prefix,
                  rpl_completer_fun_t * fun,
                  rpl_is_char_class_fun_t * is_word_char)
{
	if (is_word_char == NULL)
		is_word_char = &rpl_char_is_nonseparator;
...
// word completion callback
static bool
qword_add_completion_ex(rpl_env_t * env, void *closure, const char *replacement,
                        const char *display, const char *help,
                        long delete_before, long delete_after)
{
...
rpl_public void
rpl_complete_qword(rpl_completion_env_t * cenv, const char *prefix,
                   rpl_completer_fun_t * fun,
                   rpl_is_char_class_fun_t * is_word_char)
{
...
rpl_public void
rpl_complete_qword_ex(rpl_completion_env_t * cenv, const char *prefix,
                      rpl_completer_fun_t * fun,
                      rpl_is_char_class_fun_t * is_word_char, char escape_char,
                      const char *quote_chars)
{
	if (is_word_char == NULL)
		is_word_char = &rpl_char_is_nonseparator;
	...
	if (quote_chars[0] != 0) {
		...
		pos = 0;
		while (pos < len) {
			if (prefix[pos] == escape_char && prefix[pos + 1] != 0 && !(*is_word_char) (prefix + pos + 1, 1))   // strchr(non_word_char, prefix[pos+1]) != NULL
			{
	...
	if (quote == 0) {
		...
		while (wpos < wlen) {
			...
			if (ofs <= 0)
				break;
			if (word[wpos] == escape_char && word[wpos + 1] != 0 && !(*is_word_char) (word + wpos + 1, (long)ofs))  // strchr(non_word_char, word[wpos+1]) != NULL) {
			{
				rpl_memmove(word + wpos, word + wpos + 1,
...
rpl_public void
rpl_complete_filename(rpl_completion_env_t * cenv, const char *prefix,
                      char dir_sep, const char *roots, const char *extensions)
{
	if (roots == NULL)
		roots = ".";
	if (extensions == NULL)
