...
rpl_private void bbcode_style_add( bbcode_t* bb, const char* style_name, attr_t attr ) {
...
static bool bbcode_close( bbcode_t* bb, ssize_t base, const char* name, tag_t* pprev ) {
  ...
  while (bb->tags_nesting > base) {
    ...
    if (name==NULL || prev.name==NULL || rpl_stricmp(prev.name,name) == 0) {
      // matched
      ...
      if (name != NULL) {
        for( ssize_t i = bb->tags_nesting - 1; i > base; i--) {
          if (bb->tags[i].name != NULL && rpl_stricmp(bb->tags[i].name, name) == 0) {
            has_open_tag = true;
            break;
          }
        }
...
static const char* attr_update_color( const char* fname, rpl_color_t* field, const char* value ) {
    ...
    if (value[i] == ';') {
      ...
      if (value[i] == ';') {
        ...
        if (value[i] == ';') {
          i++; len = 0;
          while( value[i+len] != ';' && value[i+len] != 0 ) { len++; }
          if ((len == 2 && rpl_istarts_with(value+i,"on")) || (len == 1 && value[i] == '1')) { width.dots = true; }
          i += len;
        }
      }
    }
...
static const char* attr_update_ansi_color( const char* fname, rpl_color_t* color, const char* value ) {
  ssize_t num = 0;
...
static const char* attr_update_property( tag_t* tag, const char* attr_name, const char* value ) {
  ...
  if (strcmp(attr_name,fname) == 0) {    
    ...
    if (b != RPL_NONE) { tag->attr.x.bold = b; }
    return fname;
  }
  fname = "italic";
  if (strcmp(attr_name,fname) == 0) {    
    signed int b = RPL_NONE;      
    attr_update_bool(fname,&b, value); 
    if (b != RPL_NONE) { tag->attr.x.italic = b; }
    return fname;
  }
  fname = "underline";
  if (strcmp(attr_name,fname) == 0) {  
    signed int b = RPL_NONE;        
    attr_update_bool(fname,&b, value); 
    if (b != RPL_NONE) { tag->attr.x.underline = b; }
    return fname;
  }
  fname = "reverse";
  if (strcmp(attr_name,fname) == 0) {
    signed int b = RPL_NONE;          
    attr_update_bool(fname,&b, value); 
    if (b != RPL_NONE) { tag->attr.x.reverse = b; }
    return fname;
  }
  fname = "color";
  if (strcmp(attr_name,fname) == 0) {
    ...
    if (color != RPL_COLOR_NONE) { tag->attr.x.color = color; }
    return fname;
  }
  fname = "bgcolor";
  if (strcmp(attr_name,fname) == 0) {
    unsigned int color = RPL_COLOR_NONE;
    attr_update_color(fname, &color, value);
    if (color != RPL_COLOR_NONE) { tag->attr.x.bgcolor = color; }
    return fname;
  }
  fname = "ansi-sgr";
  if (strcmp(attr_name,fname) == 0) {
...
static const style_t builtin_styles[] = {
  { "b",  { { RPL_COLOR_NONE, RPL_ON  , RPL_NONE, RPL_COLOR_NONE, RPL_NONE, RPL_NONE } } },
  { "r",  { { RPL_COLOR_NONE, RPL_NONE, RPL_ON  , RPL_COLOR_NONE, RPL_NONE, RPL_NONE } } },
  { "u",  { { RPL_COLOR_NONE, RPL_NONE, RPL_NONE, RPL_COLOR_NONE, RPL_ON  , RPL_NONE } } },
  { "i",  { { RPL_COLOR_NONE, RPL_NONE, RPL_NONE, RPL_COLOR_NONE, RPL_NONE, RPL_ON   } } },
  ...
  { NULL, { { RPL_COLOR_NONE, RPL_NONE, RPL_NONE, RPL_COLOR_NONE, RPL_NONE, RPL_NONE } } }
};
...
rpl_private const char* parse_skip_white(const char* s) {
  while( *s != 0 && *s != ']') {
    if (!(*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')) break;
    s++;
  }
...
rpl_private const char* parse_skip_to_white(const char* s) {
  while( *s != 0 && *s != ']') {  
    if (*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r') break;
    s++;
  }
...
rpl_private const char* parse_skip_to_end(const char* s) {
  while( *s != 0 && *s != ']' ) { s++; }    
  return s;
}

rpl_private const char* parse_attr_name(const char* s) {
  if (*s == '#') {
...
rpl_private const char* parse_value(const char* s, const char** start, const char** end) {
  if (*s == '"') {
    s++;
    *start = s;
    while( *s != 0 ) {
...
rpl_private const char* parse_tag_value( tag_t* tag, char* idbuf, const char* s, const style_t* styles, ssize_t scount ) {
  // parse: \s*[\w-]+\s*(=\s*<value>)
...
rpl_private void bbcode_style_def( bbcode_t* bb, const char* style_name, const char* s ) {
  tag_t tag;
...
static void bbcode_restrict_width( ssize_t start, width_t width, stringbuf_t* out, attrbuf_t* attr_out ) {
  ...
  if (w > width.w) {
    ...
    const ssize_t pad_left  = (width.align == RPL_ALIGN_RIGHT ? diff : (width.align == RPL_ALIGN_LEFT  ? 0 : diff / 2));
    const ssize_t pad_right = (width.align == RPL_ALIGN_LEFT  ? diff : (width.align == RPL_ALIGN_RIGHT ? 0 : diff - pad_left));
...
rpl_private ssize_t bbcode_process_tag( bbcode_t* bb, const char* s, const ssize_t nesting_base, 
...
rpl_private void bbcode_append( bbcode_t* bb, const char* s, stringbuf_t* out, attrbuf_t* attr_out ) {
