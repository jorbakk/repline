...
rpl_private bbcode_t *
bbcode_new(alloc_t * mem, term_t * term)
{
...
rpl_private void
bbcode_style_add(bbcode_t * bb, const char *style_name, attr_t attr)
{
...
static ssize_t
bbcode_tag_push(bbcode_t * bb, const tag_t * tag)
{
...
static void
bbcode_tag_pop(bbcode_t * bb, tag_t * tag)
{
...
static void
bbcode_invalid(const char *fmt, ...)
{
	if (getenv("REPLINE_BBCODE_DEBUG") != NULL) {
		va_list args;
		va_start(args, fmt);
...
static attr_t
bbcode_open(bbcode_t * bb, ssize_t out_pos, const tag_t * tag, attr_t current)
{
...
static bool
bbcode_close(bbcode_t * bb, ssize_t base, const char *name, tag_t * pprev)
{
	// pop until match
			...
			if (name != NULL) {
				for (ssize_t i = bb->tags_nesting - 1; i > base; i--) {
					if (bb->tags[i].name != NULL
					    && rpl_stricmp(bb->tags[i].name, name) == 0) {
						has_open_tag = true;
						break;
					}
				}
...
static const char *
attr_update_bool(const char *fname, signed int *field, const char *value)
{
...
static const char *
attr_update_color(const char *fname, rpl_color_t * field, const char *value)
{
	if (value == NULL || value[0] == 0 || strcmp(value, "none") == 0) {
		*field = RPL_COLOR_NONE;
		return fname;
	}
	// hex value
	if (value[0] == '#') {
	...
	// search color names
	ssize_t lo = 0;
	ssize_t hi = RPL_HTML_COLOR_COUNT - 1;
	while (lo <= hi) {
		ssize_t mid = (lo + hi) / 2;
...
static const char *
attr_update_sgr(const char *fname, attr_t * attr, const char *value)
{
	*attr = attr_update_with(*attr, attr_from_sgr(value, rpl_strlen(value)));
	return fname;
}

static void
attr_update_width(width_t * pwidth, char default_fill, const char *value)
{
	// parse width value: <width>;<left|center|right>;<fill>;<cut>
	width_t width;
	memset(&width, 0, sizeof(width));
	width.fill = default_fill;  // use 0 for no-fill (as for max-width)
	if (rpl_atoz(value, &width.w)) {
		ssize_t i = 0;
		...
		if (value[i] == ';') {
			...
			if (len == 4 && rpl_istarts_with(value + i, "left")) {
				width.align = RPL_ALIGN_LEFT;
			} else if (len == 5 && rpl_istarts_with(value + i, "right")) {
				width.align = RPL_ALIGN_RIGHT;
			} else if (len == 6 && rpl_istarts_with(value + i, "center")) {
				width.align = RPL_ALIGN_CENTER;
			}
			i += len;
			if (value[i] == ';') {
				i++;
				...
				if (value[i] == ';') {
					...
					if ((len == 2 && rpl_istarts_with(value + i, "on"))
					    || (len == 1 && value[i] == '1')) {
						width.dots = true;
					}
					i += len;
...
static const char *
attr_update_ansi_color(const char *fname, rpl_color_t * color,
                       const char *value)
{
	ssize_t num = 0;
	if (rpl_atoz(value, &num) && num >= 0 && num <= 256) {
...
static const char *
attr_update_property(tag_t * tag, const char *attr_name, const char *value)
{
	const char *fname = NULL;
	fname = "bold";
	if (strcmp(attr_name, fname) == 0) {
		signed int b = RPL_NONE;
	...
	fname = "color";
	if (strcmp(attr_name, fname) == 0) {
		unsigned int color = RPL_COLOR_NONE;
		attr_update_color(fname, &color, value);
		if (color != RPL_COLOR_NONE) {
	...
	fname = "ansi-color";
	if (strcmp(attr_name, fname) == 0) {
		rpl_color_t color = RPL_COLOR_NONE;;
...
static const style_t builtin_styles[] = {
	{"b",
	 {{RPL_COLOR_NONE, RPL_ON, RPL_NONE, RPL_COLOR_NONE, RPL_NONE, RPL_NONE}}},
	{"r",
	 {{RPL_COLOR_NONE, RPL_NONE, RPL_ON, RPL_COLOR_NONE, RPL_NONE, RPL_NONE}}},
	{"u",
	 {{RPL_COLOR_NONE, RPL_NONE, RPL_NONE, RPL_COLOR_NONE, RPL_ON, RPL_NONE}}},
	{"i",
	 {{RPL_COLOR_NONE, RPL_NONE, RPL_NONE, RPL_COLOR_NONE, RPL_NONE, RPL_ON}}},
	{"em", {{RPL_COLOR_NONE, RPL_ON, RPL_NONE, RPL_COLOR_NONE, RPL_NONE, RPL_NONE}}},   // bold
	{"url", {{RPL_COLOR_NONE, RPL_NONE, RPL_NONE, RPL_COLOR_NONE, RPL_ON, RPL_NONE}}},  // underline
	{NULL,
	 {{RPL_COLOR_NONE, RPL_NONE, RPL_NONE, RPL_COLOR_NONE, RPL_NONE, RPL_NONE}}}
};

static void
attr_update_with_styles(tag_t * tag, const char *attr_name, const char *value,
...
rpl_private attr_t
bbcode_style(bbcode_t * bb, const char *style_name)
{
	tag_t tag;
	tag_init(&tag);
...
rpl_private const char *
parse_skip_to_white(const char *s)
{
	while (*s != 0 && *s != ']') {
		if (*s == ' ' || *s == '\t' || *s == '\n' || *s == '\r')
			break;
...
rpl_private const char *
parse_skip_to_end(const char *s)
{
	while (*s != 0 && *s != ']') {
		s++;
	}
...
rpl_private const char *
parse_value(const char *s, const char **start, const char **end)
{
	if (*s == '"') {
		s++;
		*start = s;
...
rpl_private const char *
parse_tag_value(tag_t * tag, char *idbuf, const char *s, const style_t * styles,
                ssize_t scount)
{
	// parse: \s*[\w-]+\s*(=\s*<value>)
	...
	if (idend - id == 2 && rpl_strnicmp(id, "on", 2) == 0 && *s != '=') {
		usebgcolor = true;
		id = s;
	...
	char valbuf[128];
	rpl_strncpy(idbuf, 128, id, idend - id);
	rpl_strncpy(valbuf, 128, val, valend - val);
	rpl_str_tolower(idbuf);
	rpl_str_tolower(valbuf);
	attr_update_with_styles(tag, idbuf, valbuf, usebgcolor, styles, scount);
	return s;
}

static const char *
parse_tag_values(tag_t * tag, char *idbuf, const char *s,
...
static const char *
parse_tag(tag_t * tag, char *idbuf, bool *open, bool *pre, const char *s,
          const style_t * styles, ssize_t scount)
{
	*open = true;
	*pre = false;
...
static void
bbcode_parse_tag_content(bbcode_t * bb, const char *s, tag_t * tag)
{
	tag_init(tag);
	if (s != NULL) {
		char idbuf[128];
...
rpl_private void
bbcode_style_def(bbcode_t * bb, const char *style_name, const char *s)
{
	tag_t tag;
...
rpl_private void
bbcode_style_open(bbcode_t * bb, const char *fmt)
{
	tag_t tag;
...
rpl_private void
bbcode_style_close(bbcode_t * bb, const char *fmt)
{
...
static void
bbcode_restrict_width(ssize_t start, width_t width, stringbuf_t * out,
                      attrbuf_t * attr_out)
{
	if (width.w <= 0)
		return;
	...
	if (w > width.w) {
		...
		if (width.align == RPL_ALIGN_RIGHT) {
			// right align
		...
		const ssize_t pad_left =
		    (width.align ==
		     RPL_ALIGN_RIGHT ? diff : (width.align ==
		                               RPL_ALIGN_LEFT ? 0 : diff / 2));
		const ssize_t pad_right =
		    (width.align ==
		     RPL_ALIGN_LEFT ? diff : (width.align ==
		                              RPL_ALIGN_RIGHT ? 0 : diff - pad_left));
...
rpl_private ssize_t
bbcode_process_tag(bbcode_t * bb, const char *s, const ssize_t nesting_base,
...
rpl_private void
bbcode_append(bbcode_t * bb, const char *s, stringbuf_t * out,
              attrbuf_t * attr_out)
{
	if (bb == NULL || s == NULL)
		return;
...
rpl_private void
bbcode_print(bbcode_t * bb, const char *s)
{
...
rpl_private void
bbcode_println(bbcode_t * bb, const char *s)
{
	bbcode_print(bb, s);
...
rpl_private void
bbcode_vprintf(bbcode_t * bb, const char *fmt, va_list args)
{
...
rpl_private void
bbcode_printf(bbcode_t * bb, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
rpl_private ssize_t
bbcode_column_width(bbcode_t * bb, const char *s)
{
	if (s == NULL || s[0] == 0)
		return 0;
	if (bb->vout == NULL) {
