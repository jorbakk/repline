...
rpl_private ssize_t str_skip_until_fit( const char* s, ssize_t max_width ) {
  if (s == NULL) return 0;
...
rpl_private ssize_t str_take_while_fit( const char* s, ssize_t max_width) {
  if (s == NULL) return 0;
...
rpl_private ssize_t str_prev_ofs( const char* s, ssize_t pos, ssize_t* width ) {
  ssize_t ofs = 0;
  if (s != NULL && pos > 0) {
    ofs = 1;
    while (pos > ofs) {
...
rpl_private bool skip_esc( const char* s, ssize_t len, ssize_t* esclen ) {  
...
rpl_private ssize_t str_next_ofs( const char* s, ssize_t len, ssize_t pos, ssize_t* cwidth ) {
  ssize_t ofs = 0;
  if (s != NULL && len > pos) {
...
static ssize_t str_find_backward( const char* s, ssize_t len, ssize_t pos, rpl_is_char_class_fun_t* match, bool skip_immediate_matches ) {
  if (pos > len) pos = len;
  if (pos < 0) pos = 0;
  ssize_t i = pos;
...
static ssize_t str_find_forward( const char* s, ssize_t len, ssize_t pos, rpl_is_char_class_fun_t* match, bool skip_immediate_matches ) {
...
static ssize_t str_find_word_start( const char* s, ssize_t len, ssize_t pos) {
  ssize_t start = str_find_backward(s,len,pos, &rpl_char_is_idletter,true /* skip immediate matches */);
  return (start < 0 ? 0 : start); 
}

static ssize_t str_find_word_end( const char* s, ssize_t len, ssize_t pos) {
  ssize_t end = str_find_forward(s,len,pos,&rpl_char_is_idletter,true /* skip immediate matches */);
  return (end < 0 ? len : end); 
}

static ssize_t str_find_ws_word_start( const char* s, ssize_t len, ssize_t pos) {
  ssize_t start = str_find_backward(s,len,pos,&rpl_char_is_white,true /* skip immediate matches */);
  return (start < 0 ? 0 : start); 
}

static ssize_t str_find_ws_word_end( const char* s, ssize_t len, ssize_t pos) {
  ssize_t end = str_find_forward(s,len,pos,&rpl_char_is_white,true /* skip immediate matches */);
  return (end < 0 ? len : end); 
}
...
{
  rpl_unused(is_wrap); rpl_unused(startw);
...
static bool str_set_pos_iter(
    ...
    ssize_t startw, bool is_wrap, const void* arg, void* res)
{
  rpl_unused(arg); rpl_unused(is_wrap); rpl_unused(startw);
...
rpl_private ssize_t sbuf_append_vprintf(stringbuf_t* sb, const char* fmt, va_list args) {
...
rpl_private ssize_t sbuf_insert_at_n(stringbuf_t* sbuf, const char* s, ssize_t n, ssize_t pos ) {
...
rpl_private ssize_t sbuf_insert_at(stringbuf_t* sbuf, const char* s, ssize_t pos ) {
  return sbuf_insert_at_n( sbuf, s, rpl_strlen(s), pos );
}

rpl_private ssize_t sbuf_insert_char_at(stringbuf_t* sbuf, char c, ssize_t pos ) {
  char s[2];
  s[0] = c;
  s[1] = 0;
...
rpl_private ssize_t sbuf_insert_unicode_at(stringbuf_t* sbuf, unicode_t u, ssize_t pos) {
  uint8_t s[5];
...
rpl_private void sbuf_delete_at( stringbuf_t* sbuf, ssize_t pos, ssize_t count ) {
  ...
  rpl_memmove(sbuf->buf + pos, sbuf->buf + pos + count, sbuf->count - pos - count);
  sbuf->count -= count;
  sbuf->buf[sbuf->count] = 0;
}

rpl_private void sbuf_delete_from_to( stringbuf_t* sbuf, ssize_t pos, ssize_t end ) {
  if (end <= pos) return;
...
rpl_private ssize_t sbuf_next_ofs( stringbuf_t* sbuf, ssize_t pos, ssize_t* cwidth ) {
  return str_next_ofs( sbuf->buf, sbuf->count, pos, cwidth);
}

rpl_private ssize_t sbuf_prev_ofs( stringbuf_t* sbuf, ssize_t pos, ssize_t* cwidth ) {
  return str_prev_ofs( sbuf->buf, pos, cwidth);
}

rpl_private ssize_t sbuf_next( stringbuf_t* sbuf, ssize_t pos, ssize_t* cwidth) {
...
rpl_private ssize_t sbuf_prev( stringbuf_t* sbuf, ssize_t pos, ssize_t* cwidth) {
...
rpl_private ssize_t sbuf_get_pos_at_rc( stringbuf_t* sbuf, ssize_t termw, ssize_t promptw, ssize_t cpromptw, ssize_t row, ssize_t col ) {
  return str_get_pos_at_rc( sbuf->buf, sbuf->count, termw, promptw, cpromptw, row, col);
}

// get row/col for a given position
rpl_private ssize_t sbuf_get_rc_at_pos( stringbuf_t* sbuf, ssize_t termw, ssize_t promptw, ssize_t cpromptw, ssize_t pos, rowcol_t* rc ) {
  return str_get_rc_at_pos( sbuf->buf, sbuf->count, termw, promptw, cpromptw, pos, rc);
}

rpl_private ssize_t sbuf_get_wrapped_rc_at_pos( stringbuf_t* sbuf, ssize_t termw, ssize_t newtermw, ssize_t promptw, ssize_t cpromptw, ssize_t pos, rowcol_t* rc ) {
  return str_get_wrapped_rc_at_pos( sbuf->buf, sbuf->count, termw, newtermw, promptw, cpromptw, pos, rc);
}

rpl_private ssize_t sbuf_for_each_row( stringbuf_t* sbuf, ssize_t termw, ssize_t promptw, ssize_t cpromptw, row_fun_t* fun, void* arg, void* res ) {
...
rpl_private char* sbuf_strdup_from_utf8(stringbuf_t* sbuf) {
...
rpl_public long rpl_prev_char( const char* s, long pos ) {
  ssize_t len = rpl_strlen(s);
...
rpl_public long rpl_next_char( const char* s, long pos ) {
  ssize_t len = rpl_strlen(s);
  if (pos < 0 || pos > len) return -1;
...
rpl_private bool rpl_atoz(const char* s, ssize_t* pi) {
  return (sscanf(s, "%zd", pi) == 1);
}

// parse two decimals separated by a semicolon 
rpl_private bool rpl_atoz2(const char* s, ssize_t* pi, ssize_t* pj) {
...
rpl_public bool rpl_char_is_white(const char* s, long len) {
...
rpl_public bool rpl_char_is_nonwhite(const char* s, long len) {
  return !rpl_char_is_white(s, len);
}

// Convenience: character class for separators `[ \t\r\n,.;:/\\\(\)\{\}\[\]]`.
rpl_public bool rpl_char_is_separator(const char* s, long len) {
  if (s == NULL || len != 1) return false;
  const char c = *s;
  return (strchr(" \t\r\n,.;:/\\(){}[]", c) != NULL);
}
...
rpl_public bool rpl_char_is_nonseparator(const char* s, long len) {
  return !rpl_char_is_separator(s, len);
}


// Convenience: character class for digits (`[0-9]`).
rpl_public bool rpl_char_is_digit(const char* s, long len) {
  if (s == NULL || len != 1) return false;
  const char c = *s;
  return (c >= '0' && c <= '9');
}
...
rpl_public bool rpl_char_is_hexdigit(const char* s, long len) {
  if (s == NULL || len != 1) return false;
  const char c = *s;
...
rpl_public bool rpl_char_is_letter(const char* s, long len) {
...
rpl_public bool rpl_char_is_idletter(const char* s, long len) {
  if (s == NULL || len <= 0) return false;
  const char c = *s;
...
rpl_public bool rpl_char_is_filename_letter(const char* s, long len) {
  if (s == NULL || len <= 0) return false;
  const char c = *s;
...
rpl_public long rpl_is_token(const char* s, long pos, rpl_is_char_class_fun_t* is_token_char) {
...
static int rpl_strncmp(const char* s1, const char* s2, ssize_t n) {
...
rpl_public long rpl_match_token(const char* s, long pos, rpl_is_char_class_fun_t* is_token_char, const char* token) {
  long n = rpl_is_token(s, pos, is_token_char);
  if (n > 0 && token != NULL && n == rpl_strlen(token) && rpl_strncmp(s + pos, token, n) == 0) {
    return n;
  }
  else {
    return 0;
...
rpl_public long rpl_match_any_token(const char* s, long pos, rpl_is_char_class_fun_t* is_token_char, const char** tokens) {
  long n = rpl_is_token(s, pos, is_token_char);
