...
static ssize_t
utf8_char_width(const char *s, ssize_t n)
{
	if (n <= 0)
		return 0;
...
static ssize_t
char_column_width(const char *s, ssize_t n)
{
	if (s == NULL || n <= 0)
		return 0;
	else if ((uint8_t) (*s) < ' ')
...
static ssize_t
str_column_width_n(const char *s, ssize_t len)
{
	if (s == NULL || len <= 0)
		return 0;
	ssize_t pos = 0;
...
{
	return str_column_width_n(s, rpl_strlen(s));
}

rpl_private ssize_t
str_skip_until_fit(const char *s, ssize_t max_width)
{
	if (s == NULL)
		return 0;
...
rpl_private ssize_t
str_take_while_fit(const char *s, ssize_t max_width)
{
	if (s == NULL)
		return 0;
	const ssize_t len = rpl_strlen(s);
	ssize_t pos = 0;
	ssize_t next;
	ssize_t cw;
	ssize_t cwidth = 0;
...
rpl_private ssize_t
str_prev_ofs(const char *s, ssize_t pos, ssize_t * width)
{
	ssize_t ofs = 0;
	if (s != NULL && pos > 0) {
		ofs = 1;
...
rpl_private bool
skip_esc(const char *s, ssize_t len, ssize_t * esclen)
{
...
rpl_private ssize_t
str_next_ofs(const char *s, ssize_t len, ssize_t pos, ssize_t * cwidth)
{
	ssize_t ofs = 0;
	if (s != NULL && len > pos) {
...
static ssize_t
str_limit_to_length(const char *s, ssize_t n)
{
	ssize_t i;
...
static bool
char_is_linefeed(const char *s, long n)
{
	return (n == 1 && (*s == '\n' || *s == 0));
}

static ssize_t
str_find_line_start(const char *s, ssize_t len, ssize_t pos)
{
	ssize_t start =
	    str_find_backward(s, len, pos, &char_is_linefeed,
	                      false /* don't skip immediate matches */ );
	return (start < 0 ? 0 : start);
}

static ssize_t
str_find_line_end(const char *s, ssize_t len, ssize_t pos)
{
...
static ssize_t
str_find_word_start(const char *s, ssize_t len, ssize_t pos)
{
	ssize_t start =
	    str_find_backward(s, len, pos, &rpl_char_is_idletter,
	                      true /* skip immediate matches */ );
	return (start < 0 ? 0 : start);
}

static ssize_t
str_find_word_end(const char *s, ssize_t len, ssize_t pos)
{
	ssize_t end =
	    str_find_forward(s, len, pos, &rpl_char_is_idletter,
	                     true /* skip immediate matches */ );
	return (end < 0 ? len : end);
}

static ssize_t
str_find_ws_word_start(const char *s, ssize_t len, ssize_t pos)
{
	ssize_t start =
	    str_find_backward(s, len, pos, &rpl_char_is_white,
	                      true /* skip immediate matches */ );
	return (start < 0 ? 0 : start);
}

static ssize_t
str_find_ws_word_end(const char *s, ssize_t len, ssize_t pos)
{
	ssize_t end =
	    str_find_forward(s, len, pos, &rpl_char_is_white,
	                     true /* skip immediate matches */ );
	return (end < 0 ? len : end);
}
...
static ssize_t
str_for_each_row(const char *s, ssize_t len, ssize_t termw, ssize_t promptw,
...
static bool
str_get_current_pos_iter(const char *s,
                         ssize_t row, ssize_t row_start, ssize_t row_len,
                         ssize_t startw, bool is_wrap, const void *arg,
                         void *res)
{
	rpl_unused(is_wrap);
	rpl_unused(startw);
	...
	if (pos >= row_start && pos <= (row_start + row_len)) {
		...
		debug_msg
		    ("edit: pos iter: pos: %zd (%c), row_start: %zd, rowlen: %zd\n",
...
static ssize_t
str_get_rc_at_pos(const char *s, ssize_t len, ssize_t termw, ssize_t promptw,
...
static bool
str_get_current_wrapped_pos_iter(const char *s,
                                 ssize_t row, ssize_t row_start,
...
static ssize_t
str_get_wrapped_rc_at_pos(const char *s, ssize_t len, ssize_t termw,
...
static ssize_t
str_get_pos_at_rc(const char *s, ssize_t len, ssize_t termw, ssize_t promptw,
                  ssize_t cpromptw, ssize_t row,
...
//-------------------------------------------------------------
static bool
sbuf_ensure_extra(stringbuf_t * s, ssize_t extra)
{
	...
	ssize_t newlen =
	    (s->buflen <=
	     0 ? 120 : (s->buflen > 1000 ? s->buflen + 1000 : 2 * s->buflen));
...
static void
sbuf_init(stringbuf_t * sbuf, alloc_t * mem)
{
	sbuf->mem = mem;
	sbuf->buf = NULL;
	sbuf->buflen = 0;
...
rpl_private stringbuf_t *
sbuf_new(alloc_t * mem)
{
...
rpl_private char *
sbuf_free_dup(stringbuf_t * sbuf)
{
	if (sbuf == NULL)
		return NULL;
	char *s = NULL;
...
sbuf_string_at(stringbuf_t * sbuf, ssize_t pos)
{
...
rpl_private const char *
sbuf_string(stringbuf_t * sbuf)
{
	return sbuf_string_at(sbuf, 0);
}

rpl_private char
sbuf_char_at(stringbuf_t * sbuf, ssize_t pos)
{
...
rpl_private char *
sbuf_strdup_at(stringbuf_t * sbuf, ssize_t pos)
{
	return mem_strdup(sbuf->mem, sbuf_string_at(sbuf, pos));
}

rpl_private char *
sbuf_strdup(stringbuf_t * sbuf)
{
	return mem_strdup(sbuf->mem, sbuf_string(sbuf));
}

rpl_private ssize_t
sbuf_len(const stringbuf_t * s)
{
	if (s == NULL)
		return 0;
	return s->count;
...
rpl_private ssize_t
sbuf_append_vprintf(stringbuf_t * sb, const char *fmt, va_list args)
{
	const ssize_t min_needed = rpl_strlen(fmt);
...
rpl_private ssize_t
sbuf_appendf(stringbuf_t * sb, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
rpl_private ssize_t
sbuf_insert_at_n(stringbuf_t * sbuf, const char *s, ssize_t n, ssize_t pos)
{
	...
	if (n <= 0 || !sbuf_ensure_extra(sbuf, n))
		return pos;
	rpl_memmove(sbuf->buf + pos + n, sbuf->buf + pos, sbuf->count - pos);
	rpl_memcpy(sbuf->buf + pos, s, n);
	sbuf->count += n;
...
rpl_private stringbuf_t *
sbuf_split_at(stringbuf_t * sb, ssize_t pos)
{
...
rpl_private ssize_t
sbuf_insert_at(stringbuf_t * sbuf, const char *s, ssize_t pos)
{
	return sbuf_insert_at_n(sbuf, s, rpl_strlen(s), pos);
}

rpl_private ssize_t
sbuf_insert_char_at(stringbuf_t * sbuf, char c, ssize_t pos)
{
	char s[2];
	s[0] = c;
	s[1] = 0;
...
rpl_private ssize_t
sbuf_insert_unicode_at(stringbuf_t * sbuf, unicode_t u, ssize_t pos)
{
	uint8_t s[5];
...
rpl_private void
sbuf_delete_at(stringbuf_t * sbuf, ssize_t pos, ssize_t count)
{
	if (pos < 0 || pos >= sbuf->count)
		return;
	if (pos + count > sbuf->count)
		count = sbuf->count - pos;
	rpl_memmove(sbuf->buf + pos, sbuf->buf + pos + count,
...
rpl_private void
sbuf_delete_from_to(stringbuf_t * sbuf, ssize_t pos, ssize_t end)
{
	if (end <= pos)
		return;
...
rpl_private void
sbuf_delete_from(stringbuf_t * sbuf, ssize_t pos)
{
	sbuf_delete_at(sbuf, pos, sbuf_len(sbuf) - pos);
}

rpl_private void
sbuf_clear(stringbuf_t * sbuf)
{
	sbuf_delete_at(sbuf, 0, sbuf_len(sbuf));
}

rpl_private ssize_t
sbuf_append_n(stringbuf_t * sbuf, const char *s, ssize_t n)
{
	return sbuf_insert_at_n(sbuf, s, n, sbuf_len(sbuf));
}

rpl_private ssize_t
sbuf_append(stringbuf_t * sbuf, const char *s)
{
	return sbuf_insert_at(sbuf, s, sbuf_len(sbuf));
}

rpl_private ssize_t
sbuf_append_char(stringbuf_t * sbuf, char c)
{
	char buf[2];
	buf[0] = c;
	buf[1] = 0;
...
rpl_private void
sbuf_replace(stringbuf_t * sbuf, const char *s)
{
	sbuf_clear(sbuf);
	sbuf_append(sbuf, s);
}
...
rpl_private ssize_t
sbuf_next_ofs(stringbuf_t * sbuf, ssize_t pos, ssize_t * cwidth)
{
	return str_next_ofs(sbuf->buf, sbuf->count, pos, cwidth);
}

rpl_private ssize_t
sbuf_prev_ofs(stringbuf_t * sbuf, ssize_t pos, ssize_t * cwidth)
{
	return str_prev_ofs(sbuf->buf, pos, cwidth);
}

rpl_private ssize_t
sbuf_next(stringbuf_t * sbuf, ssize_t pos, ssize_t * cwidth)
{
...
rpl_private ssize_t
sbuf_prev(stringbuf_t * sbuf, ssize_t pos, ssize_t * cwidth)
{
...
rpl_private ssize_t
sbuf_delete_char_before(stringbuf_t * sbuf, ssize_t pos)
{
...
rpl_private void
sbuf_delete_char_at(stringbuf_t * sbuf, ssize_t pos)
{
...
rpl_private ssize_t
sbuf_swap_char(stringbuf_t * sbuf, ssize_t pos)
{
	...
	if (prev >= 63)
		return 0;
	rpl_memcpy(buf, sbuf->buf + pos - prev, prev);
	rpl_memmove(sbuf->buf + pos - prev, sbuf->buf + pos, next);
	rpl_memmove(sbuf->buf + pos - prev + next, buf, prev);
	return pos - prev;
}

rpl_private ssize_t
sbuf_find_line_start(stringbuf_t * sbuf, ssize_t pos)
{
	return str_find_line_start(sbuf->buf, sbuf->count, pos);
}

rpl_private ssize_t
sbuf_find_line_end(stringbuf_t * sbuf, ssize_t pos)
{
	return str_find_line_end(sbuf->buf, sbuf->count, pos);
}

rpl_private ssize_t
sbuf_find_word_start(stringbuf_t * sbuf, ssize_t pos)
{
	return str_find_word_start(sbuf->buf, sbuf->count, pos);
}

rpl_private ssize_t
sbuf_find_word_end(stringbuf_t * sbuf, ssize_t pos)
{
	return str_find_word_end(sbuf->buf, sbuf->count, pos);
}

rpl_private ssize_t
sbuf_find_ws_word_start(stringbuf_t * sbuf, ssize_t pos)
{
	return str_find_ws_word_start(sbuf->buf, sbuf->count, pos);
}

rpl_private ssize_t
sbuf_find_ws_word_end(stringbuf_t * sbuf, ssize_t pos)
{
...
rpl_private ssize_t
sbuf_get_pos_at_rc(stringbuf_t * sbuf, ssize_t termw, ssize_t promptw,
...
rpl_private ssize_t
sbuf_get_rc_at_pos(stringbuf_t * sbuf, ssize_t termw, ssize_t promptw,
                   ssize_t cpromptw, ssize_t pos, rowcol_t * rc)
{
...
rpl_private ssize_t
sbuf_get_wrapped_rc_at_pos(stringbuf_t * sbuf, ssize_t termw, ssize_t newtermw,
...
rpl_private ssize_t
sbuf_for_each_row(stringbuf_t * sbuf, ssize_t termw, ssize_t promptw,
...
rpl_private char *
sbuf_strdup_from_utf8(stringbuf_t * sbuf)
{
	...
	for (ssize_t i = 0; i < len;) {
		...
		if (ofs <= 0) {
			...
			ssize_t nread;
			unicode_t uchr =
			    unicode_from_qutf8((const uint8_t *)(sbuf->buf + i), ofs,
			                       &nread);
			uint8_t c;
...
rpl_public long
rpl_prev_char(const char *s, long pos)
{
	ssize_t len = rpl_strlen(s);
	if (pos < 0 || pos > len)
		return -1;
...
rpl_public long
rpl_next_char(const char *s, long pos)
{
	ssize_t len = rpl_strlen(s);
	if (pos < 0 || pos > len)
		return -1;
...
rpl_private bool
rpl_atoz(const char *s, ssize_t * pi)
{
...
rpl_private bool
rpl_atoz2(const char *s, ssize_t * pi, ssize_t * pj)
{
...
rpl_private bool
rpl_atou32(const char *s, uint32_t * pu)
{
...
rpl_public bool
rpl_char_is_white(const char *s, long len)
{
	if (s == NULL || len != 1)
		return false;
	const char c = *s;
...
rpl_public bool
rpl_char_is_nonwhite(const char *s, long len)
{
	return !rpl_char_is_white(s, len);
}

// Convenience: character class for separators `[ \t\r\n,.;:/\\\(\)\{\}\[\]]`.
rpl_public bool
rpl_char_is_separator(const char *s, long len)
{
	if (s == NULL || len != 1)
		return false;
	const char c = *s;
...
rpl_public bool
rpl_char_is_nonseparator(const char *s, long len)
{
	return !rpl_char_is_separator(s, len);
}

// Convenience: character class for digits (`[0-9]`).
rpl_public bool
rpl_char_is_digit(const char *s, long len)
{
	if (s == NULL || len != 1)
		return false;
	const char c = *s;
...
rpl_public bool
rpl_char_is_hexdigit(const char *s, long len)
{
	if (s == NULL || len != 1)
		return false;
	const char c = *s;
...
rpl_public bool
rpl_char_is_letter(const char *s, long len)
{
	if (s == NULL || len <= 0)
		return false;
	const char c = *s;
...
rpl_public bool
rpl_char_is_idletter(const char *s, long len)
{
	if (s == NULL || len <= 0)
		return false;
	const char c = *s;
...
rpl_public bool
rpl_char_is_filename_letter(const char *s, long len)
{
	if (s == NULL || len <= 0)
		return false;
	const char c = *s;
...
rpl_public long
rpl_is_token(const char *s, long pos, rpl_is_char_class_fun_t * is_token_char)
{
...
static int
rpl_strncmp(const char *s1, const char *s2, ssize_t n)
{
...
rpl_public long
rpl_match_token(const char *s, long pos,
                rpl_is_char_class_fun_t * is_token_char, const char *token)
{
	long n = rpl_is_token(s, pos, is_token_char);
	if (n > 0 && token != NULL && n == rpl_strlen(token)
	    && rpl_strncmp(s + pos, token, n) == 0) {
		return n;
	} else {
		return 0;
	}
...
rpl_public long
rpl_match_any_token(const char *s, long pos,
                    ...
                    const char **tokens)
{
	...
	for (const char **token = tokens; *token != NULL; token++) {
		if (n == rpl_strlen(*token) && rpl_strncmp(s + pos, *token, n) == 0) {
			return n;
		}
	}
	return 0;
