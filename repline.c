...
rpl_public char* rpl_readline(const char* prompt_text) 
{
...
rpl_public void rpl_printf(const char* fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
  rpl_vprintf(fmt, ap);
  va_end(ap);
...
rpl_public void rpl_vprintf(const char* fmt, va_list args) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->bbcode == NULL) return;
  bbcode_vprintf(env->bbcode, fmt, args);
}

rpl_public void rpl_print(const char* s) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->bbcode==NULL) return;
  bbcode_print(env->bbcode, s);
}

rpl_public void rpl_println(const char* s) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->bbcode==NULL) return;
  bbcode_println(env->bbcode, s);
}

void rpl_style_def(const char* name, const char* fmt) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->bbcode==NULL) return;
...
rpl_public bool rpl_async_stop(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
  if (env->tty==NULL) return false;
  return tty_async_stop(env->tty);
}

static void set_prompt_marker(rpl_env_t* env, const char* prompt_marker, const char* cprompt_marker) {
...
rpl_public const char* rpl_get_prompt_marker(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return NULL;
  return env->prompt_marker;
}

rpl_public const char* rpl_get_continuation_prompt_marker(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return NULL;
  return env->cprompt_marker;
}

rpl_public void rpl_set_prompt_marker( const char* prompt_marker, const char* cprompt_marker ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  set_prompt_marker(env, prompt_marker, cprompt_marker);
}

rpl_public bool rpl_enable_twoline_prompt( bool enable ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public bool rpl_enable_multiline( bool enable ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public bool rpl_enable_beep( bool enable ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
  return term_enable_beep(env->term, enable);
}

rpl_public bool rpl_enable_color( bool enable ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
  return term_enable_color( env->term, enable );
}

rpl_public bool rpl_enable_history_duplicates( bool enable ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
  return history_enable_duplicates(env->history, enable);
}

rpl_public void rpl_set_history(const char* fname, long max_entries ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  history_load_from(env->history, fname, max_entries );
}

rpl_public void rpl_history_remove_last(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  history_remove_last(env->history);
}

rpl_public void rpl_history_add( const char* entry ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  history_push( env->history, entry );
}

rpl_public void rpl_history_clear(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
...
rpl_public bool rpl_enable_completion_preview( bool enable ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public bool rpl_enable_multiline_indent(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public bool rpl_enable_hint(bool enable) {
...
rpl_public long rpl_set_hint_delay(long delay_ms) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public void rpl_set_tty_esc_delay(long initial_delay_ms, long followup_delay_ms ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
...
rpl_public bool rpl_enable_highlight(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public bool rpl_enable_inline_help(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public bool rpl_enable_brace_matching(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public void rpl_set_matching_braces(const char* brace_pairs) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
...
rpl_public bool rpl_enable_brace_insertion(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return false;
...
rpl_public void rpl_set_insertion_braces(const char* brace_pairs) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
...
rpl_public void rpl_set_default_highlighter(rpl_highlight_fun_t* highlighter, void* arg) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
...
rpl_public void rpl_free( void* p ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  mem_free(env->mem, p);
}

rpl_public void* rpl_malloc(size_t sz) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return NULL;
  return mem_malloc(env->mem, to_ssize_t(sz));
}

rpl_public const char* rpl_strdup( const char* s ) {
...
rpl_public void rpl_term_init(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term==NULL) return;
  term_start_raw(env->term);
}

rpl_public void rpl_term_done(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term==NULL) return;
  term_end_raw(env->term,false);
}

rpl_public void rpl_term_flush(void) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term==NULL) return;
  term_flush(env->term);
}

rpl_public void rpl_term_write(const char* s) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term == NULL) return;
  term_write(env->term, s);
}

rpl_public void rpl_term_writeln(const char* s) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term == NULL) return;
  term_writeln(env->term, s);
}

rpl_public void rpl_term_writef(const char* fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
...
rpl_public void rpl_term_vwritef(const char* fmt, va_list args) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term == NULL) return;
  term_vwritef(env->term, fmt, args);
}

rpl_public void rpl_term_reset( void )  {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term == NULL) return;
  term_attr_reset(env->term);
}

rpl_public void rpl_term_style( const char* style ) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL) return;
  if (env->term == NULL || env->bbcode == NULL) return;
  term_set_attr( env->term, bbcode_style(env->bbcode, style));
}

rpl_public int rpl_term_get_color_bits(void) {
  rpl_env_t* env = rpl_get_env(); 
  if (env==NULL || env->term==NULL) return 4;  
  return term_get_color_bits(env->term);
}

rpl_public void rpl_term_bold(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->term==NULL) return;
  term_bold(env->term, enable);
}

rpl_public void rpl_term_underline(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->term==NULL) return;
  term_underline(env->term, enable);
}

rpl_public void rpl_term_italic(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->term==NULL) return;
  term_italic(env->term, enable);
}

rpl_public void rpl_term_reverse(bool enable) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->term==NULL) return;
  term_reverse(env->term, enable);
}

rpl_public void rpl_term_color_ansi(bool foreground, int ansi_color) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->term==NULL) return;
...
rpl_public void rpl_term_color_rgb(bool foreground, uint32_t hcolor) {
  rpl_env_t* env = rpl_get_env(); if (env==NULL || env->term==NULL) return;
...
rpl_public char* rpl_readline_ex(const char* prompt_text,
                                 ...
                                 rpl_highlight_fun_t* highlighter, void* highlighter_arg )
{
  ...
  // call with current
  if (completer != NULL)   { rpl_set_default_completer(completer, completer_arg); }
  if (highlighter != NULL) { rpl_set_default_highlighter(highlighter, highlighter_arg); }
...
static void rpl_env_free(rpl_env_t* env) {
  if (env == NULL) return;
...
static rpl_env_t* rpl_env_create( rpl_malloc_fun_t* _malloc, rpl_realloc_fun_t* _realloc, rpl_free_fun_t* _free )  
{
...
rpl_private rpl_env_t* rpl_get_env(void) {  
  if (rpenv==NULL) {
    ...
    if (rpenv != NULL) { atexit( &rpl_atexit ); }
  }
  return rpenv;
}

rpl_public void rpl_init_custom_malloc( rpl_malloc_fun_t* _malloc, rpl_realloc_fun_t* _realloc, rpl_free_fun_t* _free ) {
  assert(rpenv == NULL);
  if (rpenv != NULL) {
    rpl_env_free(rpenv);    
