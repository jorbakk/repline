...
rpl_private rpl_color_t
rpl_rgb(uint32_t hex)
{
	return (rpl_color_t) (0x1000000 | (hex & 0xFFFFFF));
}
...
// Create a color from a 24-bit color value.
rpl_private rpl_color_t
rpl_rgbx(ssize_t r, ssize_t g, ssize_t b)
{
	return rpl_rgb((rpl_cap8(r) << 16) | (rpl_cap8(g) << 8) | rpl_cap8(b));
}
...
static bool
color_is_rgb(rpl_color_t color)
{
	return (color >= RPL_RGB(0));   // bit 24 is set for rgb colors
}

static void
color_to_rgb(rpl_color_t color, int *r, int *g, int *b)
{
...
rpl_private rpl_color_t
color_from_ansi256(ssize_t i)
{
	if (i >= 0 && i < 8) {
		return (RPL_ANSI_BLACK + (uint32_t) i);
	} else if (i >= 8 && i < 16) {
		return (RPL_ANSI_DARKGRAY + (uint32_t) (i - 8));
	} else if (i >= 16 && i <= 255) {
		return rpl_rgb(ansi256[i]);
	} else if (i == 256) {
		return RPL_ANSI_DEFAULT;
	} else {
		return RPL_ANSI_DEFAULT;
	}
}
...
static bool
is_grayish(int r, int g, int b)
{
	return (abs(r - g) <= 4) && (abs((r + g) / 2 - b) <= 4);
}

static bool
is_grayish_color(uint32_t rgb)
{
	int r, g, b;
	color_to_rgb(RPL_RGB(rgb), &r, &g, &b);
...
static int_least32_t
rgb_distance_rmean(uint32_t color, int r2, int g2, int b2)
{
	int r1, g1, b1;
	color_to_rgb(RPL_RGB(color), &r1, &g1, &b1);
...
static int_least32_t
rgb_distance_rbmean(uint32_t color, int r2, int g2, int b2)
{
	int r1, g1, b1;
	color_to_rgb(RPL_RGB(color), &r1, &g1, &b1);
	int_least32_t rmean = (r1 + r2) / 2;
...
typedef struct rgb_cache_s {
	...
	rpl_color_t colors[RGB_CACHE_LEN];
} rgb_cache_t;

// remember a color in the LRU cache
void
rgb_remember(rgb_cache_t * cache, rpl_color_t color, int idx)
{
	if (cache == NULL)
		return;
...
int
rgb_lookup(const rgb_cache_t * cache, rpl_color_t color)
{
	if (cache != NULL) {
...
static int
rgb_match(uint32_t * palette, int start, int len, rgb_cache_t * cache,
          rpl_color_t color)
{
	assert(color_is_rgb(color));
	// in cache?
...
static int
rgb_to_ansi256(rpl_color_t color)
{
...
static int
color_to_ansi16(rpl_color_t color)
{
...
static int
color_to_ansi8(rpl_color_t color)
{
	if (!color_is_rgb(color)) {
		return (int)color;
	} else {
...
static void
fmt_color_ansi8(char *buf, ssize_t len, rpl_color_t color, bool bg)
{
	int c = color_to_ansi8(color) + (bg ? 10 : 0);
	if (c >= 90) {
		snprintf(buf, to_size_t(len), RPL_CSI "1;%dm", c - 60);
	} else {
		snprintf(buf, to_size_t(len), RPL_CSI "22;%dm", c);
	}
}

static void
fmt_color_ansi16(char *buf, ssize_t len, rpl_color_t color, bool bg)
{
	snprintf(buf, to_size_t(len), RPL_CSI "%dm",
	         color_to_ansi16(color) + (bg ? 10 : 0));
}

static void
fmt_color_ansi256(char *buf, ssize_t len, rpl_color_t color, bool bg)
{
	if (!color_is_rgb(color)) {
		fmt_color_ansi16(buf, len, color, bg);
	} else {
		snprintf(buf, to_size_t(len), RPL_CSI "%d;5;%dm", (bg ? 48 : 38),
		         rgb_to_ansi256(color));
	}
}

static void
fmt_color_rgb(char *buf, ssize_t len, rpl_color_t color, bool bg)
{
	if (!color_is_rgb(color)) {
		fmt_color_ansi16(buf, len, color, bg);
	} else {
		int r, g, b;
		color_to_rgb(color, &r, &g, &b);
		snprintf(buf, to_size_t(len), RPL_CSI "%d;2;%d;%d;%dm", (bg ? 48 : 38),
		         r, g, b);
	}
}

static void
fmt_color_ex(char *buf, ssize_t len, palette_t palette, rpl_color_t color,
             bool bg)
{
	if (color == RPL_COLOR_NONE || palette == MONOCHROME)
		return;
	if (palette == ANSI8) {
...
static void
term_color_ex(term_t * term, rpl_color_t color, bool bg)
{
	char buf[128 + 1];
...
rpl_private void
term_color(term_t * term, rpl_color_t color)
{
	term_color_ex(term, color, false);
}

rpl_private void
term_bgcolor(term_t * term, rpl_color_t color)
{
	term_color_ex(term, color, true);
}

rpl_private void
term_append_color(term_t * term, stringbuf_t * sbuf, rpl_color_t color)
{
	char buf[128 + 1];
...
rpl_private void
term_append_bgcolor(term_t * term, stringbuf_t * sbuf, rpl_color_t color)
{
	char buf[128 + 1];
...
rpl_private int
term_get_color_bits(term_t * term)
{
	switch (term->palette) {
	case MONOCHROME:
		return 1;
