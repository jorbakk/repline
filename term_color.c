...
static uint32_t rpl_cap8(ssize_t i) {
...
static bool color_is_rgb( rpl_color_t color ) {
  return (color >= RPL_RGB(0));  // bit 24 is set for rgb colors
}

static void color_to_rgb(rpl_color_t color, int* r, int* g, int* b) {
...
void rgb_remember( rgb_cache_t* cache, rpl_color_t color, int idx ) {
...
int rgb_lookup( const rgb_cache_t* cache, rpl_color_t color ) {
  if (cache != NULL) {
...
static int rgb_match( uint32_t* palette, int start, int len, rgb_cache_t* cache, rpl_color_t color ) {
  assert(color_is_rgb(color));
  // in cache?
...
static int rgb_to_ansi256(rpl_color_t color) {
...
static int color_to_ansi16(rpl_color_t color) {
...
static int color_to_ansi8(rpl_color_t color) {
  if (!color_is_rgb(color)) {
    return (int)color;
  }
  else {
...
static void fmt_color_ansi8( char* buf, ssize_t len, rpl_color_t color, bool bg ) {
...
static void fmt_color_ansi16( char* buf, ssize_t len, rpl_color_t color, bool bg ) {
  snprintf( buf, to_size_t(len), RPL_CSI "%dm", color_to_ansi16(color) + (bg ? 10 : 0) );  
}

static void fmt_color_ansi256( char* buf, ssize_t len,  rpl_color_t color, bool bg ) {
  if (!color_is_rgb(color)) {
    fmt_color_ansi16(buf,len,color,bg);
  }
  else {
    snprintf( buf, to_size_t(len), RPL_CSI "%d;5;%dm", (bg ? 48 : 38), rgb_to_ansi256(color) );  
  }
}

static void fmt_color_rgb( char* buf, ssize_t len, rpl_color_t color, bool bg ) {
  if (!color_is_rgb(color)) {
    fmt_color_ansi16(buf,len,color,bg);
  }
  else {
    int r,g,b;
    color_to_rgb(color, &r,&g,&b);
    snprintf( buf, to_size_t(len), RPL_CSI "%d;2;%d;%d;%dm", (bg ? 48 : 38), r, g, b );  
  }
}

static void fmt_color_ex(char* buf, ssize_t len, palette_t palette, rpl_color_t color, bool bg) {
  if (color == RPL_COLOR_NONE || palette == MONOCHROME) return;
  if (palette == ANSI8) {
...
static void term_color_ex(term_t* term, rpl_color_t color, bool bg) {
  char buf[128+1];
...
rpl_private void term_append_color(term_t* term, stringbuf_t* sbuf, rpl_color_t color) {
  char buf[128+1];
  fmt_color_ex(buf,128,term->palette,color,false);
  sbuf_append(sbuf,buf);
}

rpl_private void term_append_bgcolor(term_t* term, stringbuf_t* sbuf, rpl_color_t color) {
  char buf[128+1];
