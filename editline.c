...
static void edit_get_prompt_width( rpl_env_t* env, editor_t* eb, bool in_extra, ssize_t* promptw, ssize_t* cpromptw) {
  if (in_extra) {
    *promptw = 0;
    *cpromptw = 0;
  }
...
static ssize_t edit_get_rowcol( rpl_env_t* env, editor_t* eb, rowcol_t* rc ) {
  ssize_t promptw, cpromptw;
  edit_get_prompt_width(env, eb, false, &promptw, &cpromptw);
  return sbuf_get_rc_at_pos( eb->input, eb->termw, promptw, cpromptw, eb->pos, rc );
}

static void edit_set_pos_at_rowcol( rpl_env_t* env, editor_t* eb, ssize_t row, ssize_t col ) {
  ssize_t promptw, cpromptw;
  edit_get_prompt_width(env, eb, false, &promptw, &cpromptw);
...
static bool edit_pos_is_at_row_end( rpl_env_t* env, editor_t* eb ) {
  rowcol_t rc;
...
{
  rpl_unused(res); rpl_unused(startw);
...
static void edit_refresh_rows(rpl_env_t* env, editor_t* eb, stringbuf_t* input, attrbuf_t* attrs,
...
static void edit_clear(rpl_env_t* env, editor_t* eb ) {
...
static void edit_clear_screen(rpl_env_t* env, editor_t* eb ) {
...
static bool edit_resize(rpl_env_t* env, editor_t* eb ) {
  // update dimensions
...
static void edit_refresh_hint(rpl_env_t* env, editor_t* eb) {
...
static void edit_refresh_history_hint(rpl_env_t* env, editor_t* eb) {
  if (eb->modified) {
    eb->history_idx = 0;
    eb->history_widx = 0;
    eb->history_wpos = 0;
...
static void edit_undo_restore(rpl_env_t* env, editor_t* eb) {
  editor_undo_restore(eb, true);
  edit_refresh(env,eb);
}

static void edit_redo_restore(rpl_env_t* env, editor_t* eb) {
  editor_redo_restore(eb);
  edit_refresh(env,eb);
}

static void edit_cursor_left(rpl_env_t* env, editor_t* eb) {
  ssize_t cwidth = 1;
...
static void edit_cursor_right(rpl_env_t* env, editor_t* eb) {
  ssize_t cwidth = 1;
...
static void edit_cursor_line_end(rpl_env_t* env, editor_t* eb) {
...
static void edit_cursor_line_start(rpl_env_t* env, editor_t* eb) {
...
static void edit_cursor_next_word(rpl_env_t* env, editor_t* eb) {
...
static void edit_cursor_prev_word(rpl_env_t* env, editor_t* eb) {
...
static void edit_cursor_next_ws_word(rpl_env_t* env, editor_t* eb) {
...
static void edit_cursor_prev_ws_word(rpl_env_t* env, editor_t* eb) {
...
static void edit_cursor_to_start(rpl_env_t* env, editor_t* eb) {
  eb->pos = 0; 
  edit_refresh(env,eb);
}

static void edit_cursor_to_end(rpl_env_t* env, editor_t* eb) {
  eb->pos = sbuf_len(eb->input); 
  edit_refresh(env,eb);
}


static void edit_cursor_row_up(rpl_env_t* env, editor_t* eb) {
  rowcol_t rc;
  edit_get_rowcol( env, eb, &rc);
  if (rc.row == 0) {
...
static void edit_cursor_row_down(rpl_env_t* env, editor_t* eb) {
  rowcol_t rc;
...
static void edit_cursor_match_brace(rpl_env_t* env, editor_t* eb) {
  ssize_t match = find_matching_brace( sbuf_string(eb->input), eb->pos, rpl_env_get_match_braces(env), NULL );
  if (match < 0) return;
  eb->pos = match;
  edit_refresh(env,eb);
}

static void edit_backspace(rpl_env_t* env, editor_t* eb) {
  if (eb->pos <= 0) return;
...
static void edit_delete_char(rpl_env_t* env, editor_t* eb) {
...
static void edit_delete_all(rpl_env_t* env, editor_t* eb) {
...
static void edit_delete_to_end_of_line(rpl_env_t* env, editor_t* eb) { 
  ssize_t start = sbuf_find_line_start(eb->input,eb->pos);
  if (start < 0) return;
  ssize_t end = sbuf_find_line_end(eb->input,eb->pos);
  if (end < 0) return;
...
static void edit_delete_to_start_of_line(rpl_env_t* env, editor_t* eb) {
  ssize_t start = sbuf_find_line_start(eb->input,eb->pos);
  if (start < 0) return;
  ssize_t end   = sbuf_find_line_end(eb->input,eb->pos);
  if (end < 0) return;
...
static void edit_delete_line(rpl_env_t* env, editor_t* eb) {
  ssize_t start = sbuf_find_line_start(eb->input,eb->pos);
  if (start < 0) return;
  ssize_t end   = sbuf_find_line_end(eb->input,eb->pos);
  if (end < 0) return;
...
static void edit_delete_to_start_of_word(rpl_env_t* env, editor_t* eb) {
   ssize_t start = sbuf_find_word_start(eb->input,eb->pos);
  if (start < 0) return;
  editor_start_modify(eb);
  sbuf_delete_from_to( eb->input, start, eb->pos );
...
static void edit_delete_to_end_of_word(rpl_env_t* env, editor_t* eb) {
  ssize_t end = sbuf_find_word_end(eb->input,eb->pos);
  if (end < 0) return;
  editor_start_modify(eb);
  sbuf_delete_from_to( eb->input, eb->pos, end );
...
static void edit_delete_to_start_of_ws_word(rpl_env_t* env, editor_t* eb) {
  ssize_t start = sbuf_find_ws_word_start(eb->input, eb->pos);
  if (start < 0) return;
  editor_start_modify(eb);
  sbuf_delete_from_to(eb->input, start, eb->pos);
...
static void edit_delete_to_end_of_ws_word(rpl_env_t* env, editor_t* eb) {
  ssize_t end = sbuf_find_ws_word_end(eb->input, eb->pos);
  if (end < 0) return;
  editor_start_modify(eb);
  sbuf_delete_from_to(eb->input, eb->pos, end);
...
static void edit_delete_word(rpl_env_t* env, editor_t* eb) {
  ssize_t start = sbuf_find_word_start(eb->input,eb->pos);
  if (start < 0) return;
  ssize_t end   = sbuf_find_word_end(eb->input,eb->pos);
  if (end < 0) return;
...
static void edit_swap_char( rpl_env_t* env, editor_t* eb ) { 
...
static void edit_multiline_eol(rpl_env_t* env, editor_t* eb) {
  if (eb->pos <= 0) return;
...
static void edit_insert_unicode(rpl_env_t* env, editor_t* eb, unicode_t u) {
  editor_start_modify(eb);
...
static void edit_auto_brace(rpl_env_t* env, editor_t* eb, char c) {
...
static void editor_auto_indent(editor_t* eb, const char* pre, const char* post ) {
  ...
  if (prelen > 0) {
    ...
    if (!rpl_starts_with(sbuf_string(eb->input) + eb->pos - 1 - prelen, pre)) return;
    if (!rpl_starts_with(sbuf_string(eb->input) + eb->pos, post)) return;
    eb->pos = sbuf_insert_at(eb->input, "  ", eb->pos);
    sbuf_insert_char_at(eb->input, '\n', eb->pos);
  }
}

static void edit_insert_char(rpl_env_t* env, editor_t* eb, char c) {
  editor_start_modify(eb);
...
static void edit_move_hint_to_input(rpl_env_t* env, editor_t* eb)
{
  rpl_unused(env);
...
static void edit_move_word_hint_to_input(rpl_env_t* env, editor_t* eb)
{
  rpl_unused(env);
  if (sbuf_len(eb->hint) == 0) return;
  // debug_msg("HINT BEFORE: %s\n", sbuf_string(eb->hint));
...
static void edit_move_line_hint_to_input(rpl_env_t* env, editor_t* eb)
{
  rpl_unused(env);
  if (sbuf_len(eb->hint) == 0) return;
...
static char* edit_line( rpl_env_t* env, const char* prompt_text )
{
