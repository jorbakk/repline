...
rpl_private void term_set_attr( term_t* term, attr_t attr ) {
  ...
  if (attr.x.bgcolor != term->attr.x.bgcolor && attr.x.bgcolor != RPL_COLOR_NONE) {
  ...
  if (attr.x.underline != term->attr.x.underline && attr.x.underline != RPL_NONE) {
  ...
  assert(attr.x.color == term->attr.x.color || attr.x.color == RPL_COLOR_NONE);
  assert(attr.x.bgcolor == term->attr.x.bgcolor || attr.x.bgcolor == RPL_COLOR_NONE);
  assert(attr.x.bold == term->attr.x.bold || attr.x.bold == RPL_NONE);
  assert(attr.x.reverse == term->attr.x.reverse || attr.x.reverse == RPL_NONE);
  assert(attr.x.underline == term->attr.x.underline || attr.x.underline == RPL_NONE);
...
rpl_private void term_write_formatted_n( term_t* term, const char* s, const attr_t* attrs, ssize_t len ) {
  if (attrs == NULL) {
    // write directly
    term_write(term,s);
  }
...
rpl_private term_t* term_new(alloc_t* mem, tty_t* tty, bool nocolor, bool silent, int fd_out ) 
{
  ...
  if (!term->nocolor) {
    ...
    if (rpl_contains(colorterm,"24bit") || rpl_contains(colorterm,"truecolor") || rpl_contains(colorterm,"direct")) { 
      term->palette = ANSIRGB; 
    }
    else if (rpl_contains(colorterm,"8bit") || rpl_contains(colorterm,"256color")) { term->palette = ANSI256; } 
    else if (rpl_contains(colorterm,"4bit") || rpl_contains(colorterm,"16color"))  { term->palette = ANSI16; }
    else if (rpl_contains(colorterm,"3bit") || rpl_contains(colorterm,"8color"))   { term->palette = ANSI8; }
    else if (rpl_contains(colorterm,"1bit") || rpl_contains(colorterm,"nocolor") || rpl_contains(colorterm,"monochrome")) { 
    ...
    // otherwise check for some specific terminals
      ...
      if (rpl_contains(eterm,"truecolor") || rpl_contains(eterm,"direct") || rpl_contains(colorterm,"24bit")) {
        term->palette = ANSIRGB;
      }
      else if (rpl_contains(eterm,"alacritty") || rpl_contains(eterm,"kitty")) {
        term->palette = ANSIRGB;
      }
      else if (rpl_contains(eterm,"256color") || rpl_contains(eterm,"gnome")) { 
        term->palette = ANSI256;
      }  
      else if (rpl_contains(eterm,"16color")){ term->palette = ANSI16; }
      else if (rpl_contains(eterm,"8color")) { term->palette = ANSI8; }
      else if (rpl_contains(eterm,"monochrome") || rpl_contains(eterm,"nocolor") || rpl_contains(eterm,"dumb")) { 
        term->palette = MONOCHROME; 
      }
    }
  ...
  if (env_columns != NULL) { rpl_atoz(env_columns, &term->width); }
  const char* env_lines = getenv("LINES");
  if (env_lines != NULL)   { rpl_atoz(env_lines, &term->height); }
...
static void term_set_win_attr( term_t* term, attr_t ta ) {
  ...
  if (ta.x.color != RPL_COLOR_NONE) {
    if (ta.x.color >= RPL_ANSI_BLACK && ta.x.color <= RPL_ANSI_SILVER) {
      ...
      attr = (attr & 0xFFF0) | attr_color[ta.x.color - RPL_ANSI_GRAY] | FOREGROUND_INTENSITY;
    }
  ...
  if (ta.x.bgcolor != RPL_COLOR_NONE) {
    if (ta.x.bgcolor >= RPL_ANSI_BLACK && ta.x.bgcolor <= RPL_ANSI_SILVER) {
      attr = (attr & 0xFF0F) | (WORD)(attr_color[ta.x.bgcolor - RPL_ANSI_BLACK] << 4);
    }
    else if (ta.x.bgcolor >= RPL_ANSI_GRAY && ta.x.bgcolor <= RPL_ANSI_WHITE) {
      attr = (attr & 0xFF0F) | (WORD)(attr_color[ta.x.bgcolor - RPL_ANSI_GRAY] << 4) | BACKGROUND_INTENSITY;
    } 
  ...
  if (ta.x.underline != RPL_NONE) {
    attr = (attr & ~COMMON_LVB_UNDERSCORE) | (ta.x.underline == RPL_ON ? COMMON_LVB_UNDERSCORE : 0);
  }
  if (ta.x.reverse != RPL_NONE) {
    attr = (attr & ~COMMON_LVB_REVERSE_VIDEO) | (ta.x.reverse == RPL_ON ? COMMON_LVB_REVERSE_VIDEO : 0);
  }  
  if (attr != cur_attr) {
...
static bool term_esc_query_raw( term_t* term, const char* query, char* buf, ssize_t buflen ) 
{
  ...
  bool osc = (query[1] == ']');
  if (!term_write_direct(term, query, rpl_strlen(query))) return false;
...
static bool term_get_cursor_pos( term_t* term, ssize_t* row, ssize_t* col) 
{
  ...
  if (!rpl_atoz2(buf,row,col)) return false;
  return true;
}
