...
rpl_private void
term_left(term_t * term, ssize_t n)
{
	if (n <= 0)
		return;
	term_writef(term, RPL_CSI "%zdD", n);
}

rpl_private void
term_right(term_t * term, ssize_t n)
{
	if (n <= 0)
		return;
	term_writef(term, RPL_CSI "%zdC", n);
}

rpl_private void
term_up(term_t * term, ssize_t n)
{
	if (n <= 0)
		return;
	term_writef(term, RPL_CSI "%zdA", n);
}

rpl_private void
term_down(term_t * term, ssize_t n)
{
	if (n <= 0)
		return;
	term_writef(term, RPL_CSI "%zdB", n);
}

rpl_private void
term_clear_line(term_t * term)
{
	term_write(term, "\r" RPL_CSI "K");
}

rpl_private void
term_clear_to_end_of_line(term_t * term)
{
	term_write(term, RPL_CSI "K");
}

rpl_private void
term_start_of_line(term_t * term)
{
	term_write(term, "\r");
}

rpl_private ssize_t
term_get_width(term_t * term)
{
	return term->width;
}

rpl_private ssize_t
term_get_height(term_t * term)
{
	return term->height;
}

rpl_private void
term_attr_reset(term_t * term)
{
	term_write(term, RPL_CSI "m");
}

rpl_private void
term_underline(term_t * term, bool on)
{
	term_write(term, on ? RPL_CSI "4m" : RPL_CSI "24m");
}

rpl_private void
term_reverse(term_t * term, bool on)
{
	term_write(term, on ? RPL_CSI "7m" : RPL_CSI "27m");
}

rpl_private void
term_bold(term_t * term, bool on)
{
	term_write(term, on ? RPL_CSI "1m" : RPL_CSI "22m");
}

rpl_private void
term_italic(term_t * term, bool on)
{
	term_write(term, on ? RPL_CSI "3m" : RPL_CSI "23m");
}

rpl_private void
term_writeln(term_t * term, const char *s)
{
	term_write(term, s);
	term_write(term, "\n");
}
...
rpl_private void
term_write_char(term_t * term, char c)
{
	char buf[2];
	buf[0] = c;
	buf[1] = 0;
...
rpl_private attr_t
term_get_attr(const term_t * term)
{
	return term->attr;
}

rpl_private void
term_set_attr(term_t * term, attr_t attr)
{
	if (term->nocolor)
		return;
	if (attr.x.color != term->attr.x.color && attr.x.color != RPL_COLOR_NONE) {
	...
	if (attr.x.bgcolor != term->attr.x.bgcolor
	    && attr.x.bgcolor != RPL_COLOR_NONE) {
		term_bgcolor(term, attr.x.bgcolor);
		if (term->palette < ANSIRGB && color_is_rgb(attr.x.bgcolor)) {
			term->attr.x.bgcolor = attr.x.bgcolor;
		}
	}
	if (attr.x.bold != term->attr.x.bold && attr.x.bold != RPL_NONE) {
		term_bold(term, attr.x.bold == RPL_ON);
	}
	if (attr.x.underline != term->attr.x.underline
	    && attr.x.underline != RPL_NONE) {
		term_underline(term, attr.x.underline == RPL_ON);
	}
	if (attr.x.reverse != term->attr.x.reverse && attr.x.reverse != RPL_NONE) {
		term_reverse(term, attr.x.reverse == RPL_ON);
	}
	if (attr.x.italic != term->attr.x.italic && attr.x.italic != RPL_NONE) {
		term_italic(term, attr.x.italic == RPL_ON);
	}
	assert(attr.x.color == term->attr.x.color
	       || attr.x.color == RPL_COLOR_NONE);
	assert(attr.x.bgcolor == term->attr.x.bgcolor
	       || attr.x.bgcolor == RPL_COLOR_NONE);
	assert(attr.x.bold == term->attr.x.bold || attr.x.bold == RPL_NONE);
	assert(attr.x.reverse == term->attr.x.reverse
	       || attr.x.reverse == RPL_NONE);
	assert(attr.x.underline == term->attr.x.underline
	       || attr.x.underline == RPL_NONE);
	assert(attr.x.italic == term->attr.x.italic || attr.x.italic == RPL_NONE);
}

/*
rpl_private void term_clear_lines_to_end(term_t* term) {
  term_write(term, "\r" RPL_CSI "J");
}

rpl_private void term_show_cursor(term_t* term, bool on) {
  term_write(term, on ? RPL_CSI "?25h" : RPL_CSI "?25l");
}
*/
...
rpl_private void
term_writef(term_t * term, const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
...
rpl_private void
term_vwritef(term_t * term, const char *fmt, va_list args)
{
	sbuf_append_vprintf(term->buf, fmt, args);
}

rpl_private void
term_write_formatted(term_t * term, const char *s, const attr_t * attrs)
{
	term_write_formatted_n(term, s, attrs, rpl_strlen(s));
}

rpl_private void
term_write_formatted_n(term_t * term, const char *s, const attr_t * attrs,
                       ssize_t len)
{
	if (attrs == NULL) {
		// write directly
...
rpl_private void
term_write_repeat(term_t * term, const char *s, ssize_t count)
{
	for (; count > 0; count--) {
		term_write(term, s);
	}
...
rpl_private void
term_write(term_t * term, const char *s)
{
	if (s == NULL || s[0] == 0)
		return;
...
rpl_private void
term_write_n(term_t * term, const char *s, ssize_t n)
{
	if (s == NULL || n <= 0)
		return;
...
rpl_private buffer_mode_t
term_set_buffer_mode(term_t * term, buffer_mode_t mode)
{
...
static void
term_check_flush(term_t * term, bool contains_nl)
{
...
rpl_private term_t *
term_new(alloc_t * mem, tty_t * tty, bool nocolor, bool silent, int fd_out)
{
	...
	if (!term->nocolor) {
		...
		if (rpl_contains(colorterm, "24bit")
		    || rpl_contains(colorterm, "truecolor")
		    || rpl_contains(colorterm, "direct")) {
			term->palette = ANSIRGB;
		} else if (rpl_contains(colorterm, "8bit")
		           || rpl_contains(colorterm, "256color")) {
			term->palette = ANSI256;
		} else if (rpl_contains(colorterm, "4bit")
		           || rpl_contains(colorterm, "16color")) {
			term->palette = ANSI16;
		} else if (rpl_contains(colorterm, "3bit")
		           || rpl_contains(colorterm, "8color")) {
			term->palette = ANSI8;
		} else if (rpl_contains(colorterm, "1bit")
		           || rpl_contains(colorterm, "nocolor")
		           || rpl_contains(colorterm, "monochrome")) {
		...
		// otherwise check for some specific terminals
			...
			if (rpl_contains(eterm, "truecolor")
			    || rpl_contains(eterm, "direct")
			    || rpl_contains(colorterm, "24bit")) {
				term->palette = ANSIRGB;
			} else if (rpl_contains(eterm, "alacritty")
			           || rpl_contains(eterm, "kitty")) {
				term->palette = ANSIRGB;
			} else if (rpl_contains(eterm, "256color")
			           || rpl_contains(eterm, "gnome")) {
				term->palette = ANSI256;
			} else if (rpl_contains(eterm, "16color")) {
				term->palette = ANSI16;
			} else if (rpl_contains(eterm, "8color")) {
				term->palette = ANSI8;
			} else if (rpl_contains(eterm, "monochrome")
			           || rpl_contains(eterm, "nocolor")
			           || rpl_contains(eterm, "dumb")) {
				term->palette = MONOCHROME;
			}
		}
	...
	if (env_columns != NULL) {
		rpl_atoz(env_columns, &term->width);
	}
	const char *env_lines = getenv("LINES");
	if (env_lines != NULL) {
		rpl_atoz(env_lines, &term->height);
	}
...
rpl_private bool
term_is_interactive(const term_t * term)
{
	rpl_unused(term);
...
rpl_private bool
term_enable_beep(term_t * term, bool enable)
{
...
rpl_private bool
term_enable_color(term_t * term, bool enable)
{
...
static void
term_append_esc(term_t * term, const char *const s, ssize_t len)
{
	if (s[1] == '[' && s[len - 1] == 'm') {
...
static void
term_append_utf8(term_t * term, const char *s, ssize_t len)
{
	ssize_t nread;
...
static void
term_append_buf(term_t * term, const char *s, ssize_t len)
{
	ssize_t pos = 0;
	bool newline = false;
	while (pos < len) {
...
static bool
term_write_direct(term_t * term, const char *s, ssize_t n)
{
	ssize_t count = 0;
	while (count < n) {
...
static bool
term_write_console(term_t * term, const char *s, ssize_t n)
{
	DWORD written;
...
static bool
term_get_cursor_pos(term_t * term, ssize_t * row, ssize_t * col)
{
	*row = 0;
	*col = 0;
...
static void
term_move_cursor_to(term_t * term, ssize_t row, ssize_t col)
{
	CONSOLE_SCREEN_BUFFER_INFO info;
	if (!GetConsoleScreenBufferInfo(term->hcon, &info))
		return;
...
static void
term_cursor_restore(term_t * term)
{
...
static void
term_move_cursor(term_t * term, ssize_t drow, ssize_t dcol, ssize_t n)
{
	CONSOLE_SCREEN_BUFFER_INFO info;
	if (!GetConsoleScreenBufferInfo(term->hcon, &info))
		return;
...
static void
term_cursor_visible(term_t * term, bool visible)
{
...
static void
term_erase_line(term_t * term, ssize_t mode)
{
	CONSOLE_SCREEN_BUFFER_INFO info;
	if (!GetConsoleScreenBufferInfo(term->hcon, &info))
		return;
...
static void
term_clear_screen(term_t * term, ssize_t mode)
{
	CONSOLE_SCREEN_BUFFER_INFO info;
	if (!GetConsoleScreenBufferInfo(term->hcon, &info))
		return;
	...
	if (mode == 2) {
		...
		// to cursor
		length =
		    (width * ((ssize_t) info.dwCursorPosition.Y - 1)) +
		    info.dwCursorPosition.X;
	} else {
		// from cursor
		start = info.dwCursorPosition;
		length =
		    (width * ((ssize_t) info.dwSize.Y - info.dwCursorPosition.Y)) +
...
static void
term_set_win_attr(term_t * term, attr_t ta)
{
	...
	WORD attr = cur_attr;
	if (ta.x.color != RPL_COLOR_NONE) {
		if (ta.x.color >= RPL_ANSI_BLACK && ta.x.color <= RPL_ANSI_SILVER) {
			attr = (attr & 0xFFF0) | attr_color[ta.x.color - RPL_ANSI_BLACK];
		} else if (ta.x.color >= RPL_ANSI_GRAY && ta.x.color <= RPL_ANSI_WHITE) {
			attr =
			    (attr & 0xFFF0) | attr_color[ta.x.color -
			                                 RPL_ANSI_GRAY] |
			    FOREGROUND_INTENSITY;
		} else if (ta.x.color == RPL_ANSI_DEFAULT) {
			attr = (attr & 0xFFF0) | (def_attr & 0x000F);
		}
	}
	if (ta.x.bgcolor != RPL_COLOR_NONE) {
		if (ta.x.bgcolor >= RPL_ANSI_BLACK && ta.x.bgcolor <= RPL_ANSI_SILVER) {
			attr =
			    (attr & 0xFF0F) |
			    (WORD) (attr_color[ta.x.bgcolor - RPL_ANSI_BLACK] << 4);
		} else if (ta.x.bgcolor >= RPL_ANSI_GRAY
		           && ta.x.bgcolor <= RPL_ANSI_WHITE) {
			attr =
			    (attr & 0xFF0F) |
			    (WORD) (attr_color[ta.x.bgcolor - RPL_ANSI_GRAY] << 4) |
			    BACKGROUND_INTENSITY;
		} else if (ta.x.bgcolor == RPL_ANSI_DEFAULT) {
			attr = (attr & 0xFF0F) | (def_attr & 0x00F0);
		}
	}
	if (ta.x.underline != RPL_NONE) {
		attr =
		    (attr & ~COMMON_LVB_UNDERSCORE) | (ta.x.underline ==
		                                       RPL_ON ? COMMON_LVB_UNDERSCORE :
		                                       0);
	}
	if (ta.x.reverse != RPL_NONE) {
		attr =
		    (attr & ~COMMON_LVB_REVERSE_VIDEO) | (ta.x.reverse ==
		                                          RPL_ON ?
		                                          COMMON_LVB_REVERSE_VIDEO : 0);
	}
	if (attr != cur_attr) {
...
static ssize_t
esc_param(const char *s, ssize_t def)
{
	if (*s == '?')
		s++;
	ssize_t n = def;
...
static void
esc_param2(const char *s, ssize_t * p1, ssize_t * p2, ssize_t def)
{
	if (*s == '?')
		s++;
	*p1 = def;
...
static void
term_write_esc(term_t * term, const char *s, ssize_t len)
{
	ssize_t row;
	ssize_t col;
...
static bool
term_write_direct(term_t * term, const char *s, ssize_t len)
{
...
static bool
term_esc_query_raw(term_t * term, const char *query, char *buf, ssize_t buflen)
{
	if (buf == NULL || buflen <= 0 || query[0] == 0)
		return false;
	bool osc = (query[1] == ']');
	if (!term_write_direct(term, query, rpl_strlen(query)))
		return false;
...
static bool
term_esc_query(term_t * term, const char *query, char *buf, ssize_t buflen)
{
...
term_get_cursor_pos(term_t * term, ssize_t * row, ssize_t * col)
{
	...
	if (!term_esc_query(term, "\x1B[6n", buf, 128))
		return false;
	if (!rpl_atoz2(buf, row, col))
		return false;
	return true;
}

static void
term_set_cursor_pos(term_t * term, ssize_t row, ssize_t col)
{
	term_writef(term, RPL_CSI "%zd;%zdH", row, col);
}

rpl_private bool
term_update_dim(term_t * term)
{
	ssize_t cols = 0;
	ssize_t rows = 0;
	struct winsize ws;
...
term_update_dim(term_t * term)
{
	...
	if (GetConsoleScreenBufferInfo(term->hcon, &sbinfo)) {
		cols =
		    (ssize_t) sbinfo.srWindow.Right - (ssize_t) sbinfo.srWindow.Left +
		    1;
		rows =
		    (ssize_t) sbinfo.srWindow.Bottom - (ssize_t) sbinfo.srWindow.Top +
		    1;
	}
	bool changed = (term->width != cols || term->height != rows);
	term->width = cols;
...
rpl_private void
term_start_raw(term_t * term)
{
	term->raw_enabled++;
}

rpl_private void
term_end_raw(term_t * term, bool force)
{
	if (term->raw_enabled <= 0)
		return;
	if (!force) {
...
static bool
term_esc_query_color_raw(term_t * term, int color_idx, uint32_t * color)
{
	char buf[128 + 1];
	...
	if (rgb[2] != '/') {        // 48-bit rgb, hexadecimal round to 24-bit     
		r = (r + 0x7F) / 0x100; // note: can "overflow", e.g. 0xFFFF -> 0x100. (and we need `rpl_cap8` to convert.)
		g = (g + 0x7F) / 0x100;
		b = (b + 0x7F) / 0x100;
	}
	*color = (rpl_cap8(r) << 16) | (rpl_cap8(g) << 8) | rpl_cap8(b);
...
static void
term_update_ansi16(term_t * term)
{
	...
	if (ioctl(term->fd_out, GIO_CMAP, &cmap) >= 0) {
		...
		for (ssize_t i = 0; i < 48; i += 3) {
			uint32_t color =
			    ((uint32_t) (cmap[i]) << 16) | ((uint32_t) (cmap[i + 1]) << 8) |
			    cmap[i + 2];
...
term_init_raw(term_t * term)
{
	...
	if (GetConsoleScreenBufferInfoEx(term->hcon, &info)) {
		...
		for (unsigned i = 0; i < 16; i++) {
			...
			uint32_t color = (rpl_cap8(GetRValue(cr)) << 16) | (rpl_cap8(GetGValue(cr)) << 8) | rpl_cap8(GetBValue(cr));    // COLORREF = BGR
