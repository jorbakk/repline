#pragma once
#ifndef RPL_REPLINE_H
#define RPL_REPLINE_H
...
Repline is a pure C library that can be used as an alternative to the GNU readline library.

See the [Github repository](https://github.com/jorbakk/repline#readme) 
...
/// Formatted text using [bbcode markup](https://github.com/jorbakk/repline#bbcode-format).
/// \{
...
/// For example:
/// ```
/// rpl_print("[b]bold, [i]bold and italic[/i], [red]red and bold[/][/b] default.");
/// rpl_print("[b]bold[/], [i b]bold and italic[/], [yellow on blue]yellow on blue background");
/// rpl_style_add("em","i color=#888800");
/// rpl_print("[em]emphasis");
/// ```
...
/// any results from `rpl_add_completion` are automatically quoted and escaped again.
...
/// The `is_word_char` is a set of characters that are part of a "word". Use NULL for the default (`&rpl_char_is_nonseparator`).
...
/// (resulting in returning NULL from `rpl_readline`).
