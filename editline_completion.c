...
static bool edit_complete(rpl_env_t* env, editor_t* eb, ssize_t idx) {
...
static bool edit_complete_longest_prefix(rpl_env_t* env, editor_t* eb ) {
  editor_start_modify(eb);
...
rpl_private void sbuf_append_tagged( stringbuf_t* sb, const char* tag, const char* content ) {
  sbuf_appendf(sb, "[%s]", tag);  
  sbuf_append(sb,content);
  sbuf_append(sb,"[/]");
}

static void editor_append_completion(rpl_env_t* env, editor_t* eb, ssize_t idx, ssize_t width, bool numbered, bool selected ) {
...
static void editor_append_completion2(rpl_env_t* env, editor_t* eb, ssize_t col_width, ssize_t idx1, ssize_t idx2, ssize_t selected ) {  
...
static void editor_append_completion3(rpl_env_t* env, editor_t* eb, ssize_t col_width, ssize_t idx1, ssize_t idx2, ssize_t idx3, ssize_t selected ) {  
  editor_append_completion(env, eb, idx1, col_width, true, (idx1 == selected) );
  sbuf_append( eb->extra, "  ");
  editor_append_completion(env, eb, idx2, col_width, true, (idx2 == selected));
  sbuf_append( eb->extra, "  ");
...
static ssize_t edit_completions_max_width( rpl_env_t* env, ssize_t count ) {
  ssize_t max_width = 0;
...
static void edit_completion_menu(rpl_env_t* env, editor_t* eb, bool more_available) {
...
again:
  ...
  if (c == KEY_DOWN || c == KEY_TAB) {
    ...
    if (more_available) {
      ...
      count = completions_generate(env, env->completions, sbuf_string(eb->input), eb->pos, RPL_MAX_COMPLETIONS_TO_SHOW);
    }
    rowcol_t rc;
...
static void edit_generate_completions(rpl_env_t* env, editor_t* eb, bool autotab) {
  debug_msg( "edit: complete: %zd: %s\n", eb->pos, sbuf_string(eb->input) );
  if (eb->pos < 0) return;
  ssize_t count = completions_generate(env, env->completions, sbuf_string(eb->input), eb->pos, RPL_MAX_COMPLETIONS_TO_TRY);
