...
rpl_private attr_t attr_update_with( attr_t oldattr, attr_t newattr ) {
  attr_t attr = oldattr;
  if (newattr.x.color != RPL_COLOR_NONE)   { attr.x.color = newattr.x.color; }
  if (newattr.x.bgcolor != RPL_COLOR_NONE) { attr.x.bgcolor = newattr.x.bgcolor; }
  if (newattr.x.bold != RPL_NONE)          { attr.x.bold = newattr.x.bold; }
  if (newattr.x.italic != RPL_NONE)        { attr.x.italic = newattr.x.italic; }
  if (newattr.x.reverse != RPL_NONE)       { attr.x.reverse = newattr.x.reverse; }
  if (newattr.x.underline != RPL_NONE)     { attr.x.underline = newattr.x.underline; }
  return attr;
}
...
rpl_private attr_t attr_from_sgr( const char* s, ssize_t len) {
  ...
  for( ssize_t i = 0; i < len && s[i] != 0; i++) {
    ...
    switch(cmd) {
      ...
      case  1: attr.x.bold = RPL_ON; break;
      case  3: attr.x.italic = RPL_ON; break;
      case  4: attr.x.underline = RPL_ON; break;
      case  7: attr.x.reverse = RPL_ON; break;
      case 22: attr.x.bold = RPL_OFF; break;
      case 23: attr.x.italic = RPL_OFF; break;
      case 24: attr.x.underline = RPL_OFF; break;
      case 27: attr.x.reverse = RPL_OFF; break;
      case 39: attr.x.color = RPL_ANSI_DEFAULT; break;
      case 49: attr.x.bgcolor = RPL_ANSI_DEFAULT; break;
      default: {
        if (cmd >= 30 && cmd <= 37) {
...
rpl_private void attrbuf_free( attrbuf_t* ab ) {
  if (ab==NULL) return;
  mem_free(ab->mem, ab->attrs);
  mem_free(ab->mem, ab);
}

rpl_private void attrbuf_clear(attrbuf_t* ab) {
  if (ab == NULL) return;
  ab->count = 0;
}

rpl_private ssize_t attrbuf_len( attrbuf_t* ab ) {
...
rpl_private void attrbuf_insert_at( attrbuf_t* ab, ssize_t pos, ssize_t count, attr_t attr ) {
  ...
  rpl_memmove( ab->attrs + pos + count, ab->attrs + pos, (ab->count - pos)*ssizeof(attr_t) );
  ab->count += count;
...
rpl_private void attrbuf_delete_at( attrbuf_t* ab, ssize_t pos, ssize_t count ) {
  ...
  rpl_memmove( ab->attrs + pos, ab->attrs + pos + count, ab->count - (pos + count) );
  ab->count -= count;
}
