...
static void highlighter(rpl_highlight_env_t* henv, const char* input, void* arg);
...
static void word_completer(rpl_completion_env_t* cenv, const char* word ) 
{
  ...
  if (strcmp(word,"f") == 0) {  
    ...
    rpl_add_completion_ex(cenv, "(x) => x",   "D — (x) => x",       "identity function in D");
    rpl_add_completion_ex(cenv, "\\x -> x",   "Haskell — \\x -> x", "identity_bot function in Haskell");
    rpl_add_completion_ex(cenv, "\\x => x",   "Idris — \\x => x",   "dependent identity function in Idris");
    rpl_add_completion_ex(cenv, "fn(x){ x }", "Koka — fn(x){ x }",  "total identity function in Koka");
    rpl_add_completion_ex(cenv, "fun x -> x", "Ocaml — fun x -> x", "identity lambda in OCaml");
  }
    ...
    for(int i = 0; i < 100000; i++) {
      ...
      if (!rpl_add_completion(cenv, buf)) break;  // break early if not all completions are needed (for better latency)
    }
  }
}
...
static void completer(rpl_completion_env_t* cenv, const char* input ) 
{
  // try to complete file names from the roots "." and "/usr/local"
  rpl_complete_filename(cenv, input, 0, ".;/usr/local;c:\\Program Files" , NULL /* any extension */);

  // and also use our custom completer  
  rpl_complete_word( cenv, input, &word_completer, NULL /* from default word boundary; whitespace or separator */ );        
...
static void highlighter(rpl_highlight_env_t* henv, const char* input, void* arg) {
  ...
  for (long i = 0; i < len; ) {
    ...
    if ((tlen = rpl_match_any_token(input, i, &rpl_char_is_idletter, keywords)) > 0) {
      rpl_highlight(henv, i, tlen, "keyword"); 
      i += tlen;
    }
    else if ((tlen = rpl_match_any_token(input, i, &rpl_char_is_idletter, controls)) > 0) {
      rpl_highlight(henv, i, tlen, "plum");      // html color (or use the `control` style)
      i += tlen;
    }
    else if ((tlen = rpl_match_any_token(input, i, &rpl_char_is_idletter, types)) > 0) {
