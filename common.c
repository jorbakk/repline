...
rpl_public bool rpl_starts_with( const char* s, const char* prefix ) {
...
rpl_public bool rpl_istarts_with( const char* s, const char* prefix ) {
  if (s==prefix) return true;
  if (prefix==NULL) return true;
  if (s==NULL) return false;
  ...
  ssize_t i;
  for( i = 0; s[i] != 0 && prefix[i] != 0; i++) {
    if (rpl_tolower(s[i]) != rpl_tolower(prefix[i])) return false;
  }
  return (prefix[i] == 0);
}
...
static const char* rpl_stristr(const char* s, const char* pat) {
  if (s==NULL) return NULL;
  if (pat==NULL || pat[0] == 0) return s;
  ssize_t patlen = rpl_strlen(pat);
  for (ssize_t i = 0; s[i] != 0; i++) {
    if (rpl_strnicmp(s + i, pat, patlen) == 0) return (s+i);
  }
  return NULL;
}
...
rpl_private unicode_t unicode_from_qutf8(const uint8_t* s, ssize_t len, ssize_t* count) {
  unicode_t c0 = 0;
  if (len <= 0 || s == NULL) {
    goto fail;
  }
