...
static int color_weight(rpl_color_t c) {
  return (int)(c);
}
...
static void write_html_colors(void) {
  qsort(html_colors, RPL_HTML_COLOR_COUNT, sizeof(html_colors[0]), &html_color_compare );
  rpl_print("print html colors\n");  
  for(int i = 0; i < RPL_HTML_COLOR_COUNT-1; i++) {
    rpl_printf("[width=10][bgcolor=%s]%s[/][/] ", html_colors[i].name, html_colors[i].name);
    if ((i+1)%8 == 0) rpl_print("\n\n");
  }
  rpl_println("");  
}
