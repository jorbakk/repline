...
rpl_private bool
code_is_ascii_char(code_t c, char *chr)
{
	if (c >= ' ' && c <= 0x7F) {
		if (chr != NULL)
			*chr = (char)c;
		...
		if (tty_readc_noblock(tty, buf + count, tty->esc_timeout)) {
			...
			if (c0 > 0xDF) {
				if (tty_readc_noblock(tty, buf + count, tty->esc_timeout)) {
					count++;
					if (c0 > 0xEF) {
						if (tty_readc_noblock
						    (tty, buf + count, tty->esc_timeout)) {
							count++;
						}
					}
				}
...
tty_cpush_csi_unicode(tty_t *tty, code_t mods, uint32_t unicode)
{
	if ((unicode < 0x80 && mods == 0) ||
	    (mods == KEY_MOD_CTRL && unicode < ' ' && unicode != KEY_TAB
	     ...
	     && unicode != KEY_BACKSP) || (mods == KEY_MOD_SHIFT && unicode >= ' '
	                                   && unicode <= KEY_RUBOUT)) {
...
tty_init_utf8(tty_t *tty)
{
	...
	tty->is_utf8 = (rpl_icontains(loc, "UTF-8") || rpl_icontains(loc, "utf8")
	                || rpl_stricmp(loc, "C") == 0);
...
tty_set_esc_delay(tty_t *tty, long initial_delay_ms, long followup_delay_ms)
{
	tty->esc_initial_timeout =
	    (initial_delay_ms <
	     0 ? 0 : (initial_delay_ms > 1000 ? 1000 : initial_delay_ms));
	tty->esc_timeout =
	    (followup_delay_ms <
	     0 ? 0 : (followup_delay_ms > 1000 ? 1000 : followup_delay_ms));
}
...
static bool
sigaction_is_valid(struct sigaction *sa)
{
