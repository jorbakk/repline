...
rpl_private bool tty_read_timeout(tty_t* tty, long timeout_ms, code_t* code) 
{
...
static bool tty_init_utf8(tty_t* tty) {
  ...
  tty->is_utf8 = (rpl_icontains(loc,"UTF-8") || rpl_icontains(loc,"utf8") || rpl_stricmp(loc,"C") == 0);
...
rpl_private bool tty_term_resize_event(tty_t* tty) {
  if (tty == NULL) return true;
...
#if defined(TIOCSTI) 
rpl_private bool tty_async_stop(const tty_t* tty) {
...
rpl_private bool tty_readc_noblock(tty_t* tty, uint8_t* c, long timeout_ms) {  // don't modify `c` if there is no input
  // in our pushback buffer?
  if (tty_cpop(tty, c)) return true;
